...
def get_gen_ai_client() -> gen_ai_client.GenAIClient:
  """Returns the process-wide GenAIClient, built on first use."""
...
def get_gda_client() -> gemini_data_analytics_client.GeminiDataAnalyticsClient:
  """Returns the process-wide GeminiDataAnalyticsClient, built on first use.

  The project is empty because ask_question takes the project and the location
  from the agent resource name it is passed, so there is nothing useful to set
  here. Code that calls the agent management methods builds its own client
  with the agent's parent path.
  """
  global _GDA_CLIENT
...
def get_execution_service(
    ...
    gda_client: (
        gemini_data_analytics_client.GeminiDataAnalyticsClient
    ) = Depends(get_gda_client),

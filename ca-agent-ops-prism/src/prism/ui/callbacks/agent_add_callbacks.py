...
@typed_callback(
    [
        Output(AgentIds.Add.CONTAINER_BQ_DATASOURCE, CP.STYLE),
        Output(AgentIds.Add.CONTAINER_LOOKER_DATASOURCE, CP.STYLE),
    ],
...
@typed_callback(
    [
        ...
        Output(AgentIds.Add.LOADING_OVERLAY, CP.VISIBLE),
        Output(NOTIFICATION_CONTAINER, "sendNotifications"),
    ],
  ...
  try:
    ...
    return (
        ...
        [{
            ...
            # in agent_detail_callbacks.py.
            "message": (
                "Could not create the agent. The details are in the server log."
            ),
            "color": "red",
            "autoClose": False,
        }],
...
dash.clientside_callback(
    ...
    """,
    Output(AgentIds.Add.LOADING_OVERLAY, CP.VISIBLE, allow_duplicate=True),
    Input(AgentIds.Add.BTN_SUBMIT, CP.N_CLICKS),
    prevent_initial_call=True,
)
...
@typed_callback(
    ...
    running=[(Output(AgentIds.Form.BTN_TEST_LOOKER, CP.LOADING), True, False)],
)
def test_looker_connectivity_add(n_clicks, uri, client_id, client_secret):
  ...
  try:
    ...
    # agent_detail_callbacks.py. The message the service builds for a rejected
    # login is returned above; this branch is whatever the SDK raised on the
    # way there.
    return (
        (
...
@typed_callback(
    [
        Output(AgentIds.Form.ALERT_BQ_TEST, CP.CHILDREN),
        Output(AgentIds.Form.ALERT_BQ_TEST, CP.HIDE),
        Output(AgentIds.Form.ALERT_BQ_TEST, "color"),
    ],
    [Input(AgentIds.Form.BTN_TEST_BQ, CP.N_CLICKS)],
    [
        State(AgentIds.Form.INPUT_BQ_TABLES, CP.VALUE),
    ],
    prevent_initial_call=True,
    # See the note on test_looker_connectivity_add. One round trip per table,
    # so this is the slower of the two.
    running=[(Output(AgentIds.Form.BTN_TEST_BQ, CP.LOADING), True, False)],
)
def test_bq_tables_add(n_clicks, tables_text):
  """Checks the BQ tables against BigQuery during agent creation."""
  if not n_clicks:
    return dash.no_update, True, "blue"
  ...
  try:
    ...
    # Not the exception text. See the note in fetch_remote_config, in
    # agent_detail_callbacks.py. A per table error is reported by
    # render_bq_check_results below; this branch is the call itself failing.
    return (
